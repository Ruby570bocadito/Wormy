...
FEATURE_NAMES = [
    'scan_rate', 'stealth_delay', 'ports_scanned', 'targets_parallel',
    'is_windows_target', 'is_linux_target', 'is_dc_target',
    'has_edr', 'has_ids', 'has_av', 'is_honeypot',
    'hour_of_day', 'is_work_hours', 'day_of_week',
    'success_rate_last_10', 'polymorphic_level',
    'protocol_count', 'credential_age_hours',
]
...
def generate_data(n_samples=5000):
    ...
    for i in range(n_samples):
        risk = (X[i, 5] * 0.3 + X[i, 6] * 0.25 + X[i, 7] * 0.2 +
                X[i, 8] * 0.15 + X[i, 9] * 0.1 + X[i, 10] * 0.4 +
                X[i, 0] * 0.2 - X[i, 1] * 0.15)
