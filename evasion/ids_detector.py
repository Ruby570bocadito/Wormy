...
class IDSDetector:
    ...
    def is_honeypot(self, target: str, scan_result: Dict) -> Tuple[bool, float]:
        ...
        if hostname and any(x in hostname.lower() for x in ['honeypot', 'honey', 'trap', 'canary', 'test', 'demo']):
    ...
    def _check_honeypot_signatures(self, ports: List[int], banners: Dict) -> bool:
        ...
        honeypot_signatures = [
            'kippo', 'cowrie', 'dionaea', 'glastopf', 'honeyd',
            'conpot', 'amun', 'mwcollect', 'shockpot',
        ]
